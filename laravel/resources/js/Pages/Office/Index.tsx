...
import { ColumnsDataTable, DataOffice, PageProps, Paginate } from '@/types';
...
export default function OfficeComponent({ auth }: PageProps) {
    ...
    const [formState, setFormState] = useState<DataOffice>({} as DataOffice)
    ...
    const OpenEditModal = (data: DataOffice) => {
        setOpenModal(true);
        setFormState(data);
    }
    ...
    useEffect(() => {
        GetData(`/offices/data?search=${search}&sorting=${sorting}&offset=0`).then((data) => {
            setupData(data.data);
            setPaginate(data.links);
        });
    }, [search, sorting]);
                        ...
                        <div className="space-y-3">
                            <div className='flex gap-2'>
                                <div className='space-y-1 w-full'>
                                    <label className="text-gray-900 text-sm font-semibold">Office Name</label>
                                    <input
                                        required
                                        value={formState.name}
                                        onChange={(e) => setFormState({ ...formState, name: e.target.value })}
                                        placeholder="Office Name" className="focus:outline-none focus:ring-0 focus:border-indigo-500 border-gray-300 py-3 px-4 rounded text-wrap text-gray-900 text-xs font-semibold bg-opacity-75 w-full" />
                            ...
                            <div className='flex gap-2'>
                                <div className='space-y-1 w-full'>
                                    <label className="text-gray-900 text-sm font-semibold">Office Address</label>
                                    <input
                                        required
                                        value={formState.address}
                                        onChange={(e) => setFormState({ ...formState, address: e.target.value })}
                                        placeholder="Office Address" className="focus:outline-none focus:ring-0 focus:border-indigo-500 border-gray-300 py-3 px-4 rounded text-wrap text-gray-900 text-xs font-semibold bg-opacity-75 w-full" />
